...
def pixels_set(i, color):
    pixel_array[i] = (color[1]<<16) + (color[0]<<8) + color[2] # set 24-bit color
...
def breathing_led(color):
    ...
    for ii in breath_amps:
        for jj in range(len(pixel_array)):
            pixel_set(jj, color) # show all colors
        pixels_show(ii/255)
        time.sleep(0.02)
...
####### start RING Spiner
def ring_spinner():
    spot = (255,0,0) # looping color starts red
    wipe = (255,255,255)
    for rr in range(2000):
        ring_spin(spot,wipe)
